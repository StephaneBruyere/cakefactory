...
//	private CustomOAuth2UserService customOAuth2UserService;
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http
			.csrf().and().formLogin().loginPage("/login") // the custom login page
...
//				.and()
			.defaultSuccessUrl("/account").and().logout() // must be a POST with csrf on
			.deleteCookies("JSESSIONID")
//				.logoutSuccessUrl("/")
			.and().addFilterBefore(new BrandFilter(), CsrfFilter.class).authorizeRequests()
			.antMatchers(staticResources).permitAll().antMatchers("/", "/login", "/oauth2/**", "/signup", "/basket")
